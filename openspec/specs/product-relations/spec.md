# product-relations Specification

## Purpose
TBD - created by archiving change inventory-transfer-with-conversion. Update Purpose after archive.
...
### Requirement: Related products can be queried by SKU including stock
...
The system SHALL allow bulk import rows to include a `relatedProducts` field using comma-separated SKU entries, SHALL store imported relations with type `related`, and SHALL omit invalid or unresolved relations without failing the product row.
...
- **THEN** the imported product stores that relation with type `related`
...
- **THEN** the system resolves and stores that relation for the current product with type `related`
...
### Requirement: Related product selector shows identifying context
The system SHALL show enough information in the product-detail relationship selector for an admin to distinguish candidate products, including SKU, product name, and the configured numeric conversion attribute when available.

#### Scenario: Candidate product has conversion attribute configured with value
- **WHEN** an admin opens the related products editor and a candidate product belongs to a type with `conversionAttribute` configured and the product has a value for that attribute
- **THEN** the selector displays the option as `SKU - Name (Attribute Label: value)`

#### Scenario: Candidate product has no conversion attribute metadata
- **WHEN** an admin opens the related products editor and a candidate product has no configured conversion attribute or no value loaded for it
- **THEN** the selector still displays a stable fallback with at least SKU and product name
